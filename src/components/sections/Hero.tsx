        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          ...
          <form onSubmit={handleSearch} className="w-full max-w-xl">
            <div className="flex items-center bg-white rounded-2xl shadow-lg border border-stone-200 overflow-hidden focus-within:border-[#ee3f5c] focus-within:shadow-[0_0_0_3px_rgba(238,63,92,0.12)] transition-all duration-200">
              <Search size={20} className="ml-5 text-stone-400 shrink-0" />
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Search Products name..."
                className="flex-1 px-4 py-4 text-[#000000] placeholder-stone-400 bg-transparent outline-none text-base font-medium"
              />
              <button
                type="submit"
                className="m-1.5 bg-[#ee3f5c] hover:bg-[#c1023e] text-white px-6 py-3 rounded-xl font-semibold transition-colors shrink-0 flex items-center gap-2"
              >
                Search
                <ArrowRight size={16} />
              </button>
